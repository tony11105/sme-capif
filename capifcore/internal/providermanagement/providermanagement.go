...
func (pm *ProviderManager) GetGetAllRegistrations(ctx echo.Context) error {
	pm.lock.Lock()
	defer pm.lock.Unlock()

	var registrations []provapi.APIProviderEnrolmentDetails
	for _, provider := range pm.registeredProviders {
		registrations = append(registrations, provider)
	}

	if err := ctx.JSON(http.StatusOK, registrations); err != nil {
		// Handle the error if JSON marshaling or response sending fails
		return sendCoreError(ctx, http.StatusInternalServerError, "Failed to send registrations")
	}
	return nil
}
